...
%Taking only one sample of the test
myrecording = my2(:,1);

%plot(myrecording);
%Taking the length of the recording
length_samp=length(myrecording);

%Filter Low Pass
%Designing the low pass fiter with cutoff freq 1000
d=fdesign.lowpass('N,Fc',100,1000,fs);

 %designmethods(d);
 Hd = design(d);
 
 %fvtool(Hd);
 %Finding the impulse response of the filter
 %Filtering the input
 [h,t] = impz(Hd);
 myrecording=filter(Hd,myrecording);
 
 %Rectangular Windowing
 %30 milisecond of the frame
num_of_samples=fs*30*0.001;
% 10 milisecond of the overlapping
num_over=fs*10*0.001;
%Number of new samples in each frame
num_samp=num_of_samples-num_over;
%Find the number of iterations
n=ceil((length_samp-num_of_samples)/num_samp);


% 30 milisecond of the frame and 10 milisecond of the overlapping
...
%Taking the average zero crossing frequency of all the frames
zcr_avg=mean((zcr_sum/2));                  
